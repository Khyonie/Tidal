...
public class CommandError
{
	...
	public void display(
		...
		sender.sendMessage("§cError " + errorNumber + ": " + message + " at position " + index);
		...
		args[index] = "§e" + new StringBuilder(args[index])
			.replace(this.errorStart, this.errorEnd, "§f§n" + args[index].substring(this.errorStart, this.errorEnd) + "§e")
			.toString() + "§7"; 

		sender.sendMessage("§c§l⤷ §7/" + commandLabel + " " + Arrays.toString(args, " "));

		if (this.resolution != null)
		{
			sender.sendMessage("§9 §9 Possible fix: " + this.resolution);
		}
	}
