...
public abstract class TidalCommand extends Command
{
	private static boolean isEnabled = true;
	private static CommandMap serverCommandMap = null;
	...
	public TidalCommand(
		...
		for (Method method : this.getClass().getDeclaredMethods())
		{
			...
			try {
				...
				Bukkit.getConsoleSender().sendMessage("§cFailed to setup command using method \"" + method.getName() + "\" in class \"" + this.getClass().getName() + "\" (" + e.getMessage() + ")");
				e.printStackTrace();
				continue;
			}
		...
		throws IllegalStateException
	{
		...
		for (int i = 1; i < params.length; i++)
		{
			...
			// Auto-discover used enum types (no automatic class shading from Hyacinth)
			if (param.getType().isEnum() && !TypeManager.hasParserFor(param.getType()))
			{
				generateEnumParser((Class<? extends Enum<?>>) param.getType());
			}
	...
	public boolean execute(CommandSender sender, String commandLabel, String[] args)
	{
		...
		if (!isEnabled)
		{
			sender.sendMessage("§cTidal command execution has been temporarily disabled.");
			return true;
		}
		...
		if (args.length == 0)
		{
			...
			{
				sender.sendMessage("§cCommand \"/" + commandLabel + "\" expects at least one argument.");
				return true;
			}
			...
			try {
				...
				sender.sendMessage("§cAn exception occurred when attempting to invoke this command.");
				e.printStackTrace();
			}
			return true;
		}
	...
	public List<String> tabComplete(
		...
		if (this.tabCompleter != null)
		{
			try {
				...
				Bukkit.getConsoleSender().sendMessage("§cAn exception occurred when attempting to handle tab-completion.");
				e.printStackTrace();
			}
		}
	...
	private static void generateEnumParser(
		Class<?> type
	) {
		if (TypeManager.hasParserFor(type))
		{
			return;
		}
		...
		TypeManager.register(type, new EnumParser<>(enumType));
	}

	public void register()
	{
		// Cache command map if it isn't cached already
		if (serverCommandMap == null)
		{
			...
			try {
				...
				serverCommandMap = (CommandMap) commandMapField.get(server);
		...
		serverCommandMap.register(this.getName(), this);
	}
}
